...
class WavFile {
    ...
    /*private*/ ioState : WavFile.IOState;
    ...
    constructor() {
        ...
        this.bytesRead = 0;
        this.frameCounter = 0;
        this.buffer = (s => { let a=[]; while(s-->0) a.push(0); return a; })(WavFile.BUFFER_SIZE);
    }
    ...
    public static newWavFile(file : java.io.File, numChannels : number, numFrames : number, validBits : number, sampleRate : number) : WavFile {
        let wavFile : WavFile = new WavFile();
        wavFile.file = file;
    ...
    public static openWavFile(file : java.io.File) : WavFile {
        let wavFile : WavFile = new WavFile();
        wavFile.file = file;
        ...
        let riffChunkID : number = WavFile.getLE(wavFile.buffer, 0, 4);
        let chunkSize : number = WavFile.getLE(wavFile.buffer, 4, 4);
        let riffTypeID : number = WavFile.getLE(wavFile.buffer, 8, 4);
        if(riffChunkID !== WavFile.RIFF_CHUNK_ID) throw new WavFileException("Invalid Wav Header data, incorrect riff chunk ID");
        if(riffTypeID !== WavFile.RIFF_TYPE_ID) throw new WavFileException("Invalid Wav Header data, incorrect riff type ID");
        ...
        while((true)) {
            ...
            let chunkID : number = WavFile.getLE(wavFile.buffer, 0, 4);
            ...
            if(chunkID === WavFile.FMT_CHUNK_ID) {
                ...
                let compressionCode : number = (<number>WavFile.getLE(wavFile.buffer, 0, 2)|0);
                if(compressionCode !== 1) throw new WavFileException("Compression Code " + compressionCode + " not supported");
                wavFile.numChannels = (<number>WavFile.getLE(wavFile.buffer, 2, 2)|0);
                wavFile.sampleRate = WavFile.getLE(wavFile.buffer, 4, 4);
                wavFile.blockAlign = (<number>WavFile.getLE(wavFile.buffer, 12, 2)|0);
                wavFile.validBits = (<number>WavFile.getLE(wavFile.buffer, 14, 2)|0);
    ...
    readSample() : number {
        ...
        for(let b : number = 0; b < this.bytesPerSample; b++) {
            if(this.bufferPointer === this.bytesRead) {
                let read : number = this.iStream.read(this.buffer, 0, WavFile.BUFFER_SIZE);
    ...
    public readFrames$int_A$int$int(sampleBuffer : number[], offset : number, numFramesToRead : number) : number {
        if(this.ioState !== WavFile.IOState.READING) throw Object.defineProperty(new Error("Cannot read from WavFile instance"), '__classes', { configurable: true, value: ['java.lang.Throwable','java.io.IOException','java.lang.Object','java.lang.Exception'] });
    ...
    public writeFrames$int_A$int$int(sampleBuffer : number[], offset : number, numFramesToWrite : number) : number {
        if(this.ioState !== WavFile.IOState.WRITING) throw Object.defineProperty(new Error("Cannot write to WavFile instance"), '__classes', { configurable: true, value: ['java.lang.Throwable','java.io.IOException','java.lang.Object','java.lang.Exception'] });
    ...
    public static main(args : string[]) {
        ...
        try {
            for(let index121=0; index121 < args.length; index121++) {
                let filename = args[index121];
                {
                    let readWavFile : WavFile = WavFile.openWavFile(new java.io.File(filename));
                    readWavFile.display();
                    ...
                    let writeWavFile : WavFile = WavFile.newWavFile(new java.io.File("out.wav"), numChannels, numFrames, validBits, sampleRate);
            ...
            let writeWavFile : WavFile = WavFile.newWavFile(new java.io.File("out2.wav"), 1, 10, 23, 44100);
